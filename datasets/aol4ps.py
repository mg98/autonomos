...
def enrich_docs_with_aolia(docs_df: pd.DataFrame):
    ...
    aolia_urls = {doc.url for doc in dataset.docs_iter()}
    docs_df = docs_df[docs_df['Url'].isin(aolia_urls)].copy()

    # Create a hashmap for fast lookup
    url_to_title = {}
    for doc in dataset.docs_iter():
        url_to_title[doc.url] = {
            "title": doc.title,
            "body": doc.text
        }
    ...
    # Update titles and bodies using the hashmaps
    docs_df['Body'] = ""
    for url, info in url_to_title.items():
        if url in url_to_idx:
            for idx in url_to_idx[url]:
                if info["title"] != "":
                    docs_df.at[idx, 'Title'] = info["title"]
                docs_df.at[idx, 'Body'] = info["body"]
...
def load_dataset(path: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    enrich_docs_with_aolia(docs_df)
